...
% behavioral output
conditionNames = {'falsealarm','vowelstep1.wav','vowelstep3.wav','vowelstep5.wav','vowelstep7.wav','sinestep1.wav','sinestep3.wav','sinestep5.wav','sinestep7.wav','catch.wav'};
...
for i=1:length(sessionFiles)
    ...
    for r=1:runs
        if r==1
            ...
            % next run
            runStart = 92+(runLength*(r-1));
        end
        
        % separate data for current run
        runEnd = (runStart+runLength)-1;
        current_run = raw(runStart:runEnd,1:10);
        
        % grab onset information
        for j=1:conditions
                        
            % create a cell that just has the list of st imulus names from
            % the current block
            trialType = current_run(:,4);
            ...
            trialOnsets=current_run(find(index),10);
